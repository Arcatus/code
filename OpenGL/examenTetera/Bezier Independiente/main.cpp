...
int main()
{
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        glDrawElements(
            GL_TRIANGLES, (resx * resz * 3 * 2) , GL_UNSIGNED_INT, (GLvoid*)0);
        glBindVertexArray(0);
    ...
    (GLuint* vbo, GLuint* ebo, GLuint resx, GLuint resz, GLuint numcomp)
{
    ...
    std::ifstream iFile("indicesOK.txt",std::ios::in);
    ...
    for (int i = 0; i < 16; i++)
    {
            ...
            controlPoints[i] = Vector3{ getx, gety, getz };
    }
        ...
        for(GLuint z = 0; z <= resz; z++)
        {
            for(GLuint x = 0; x <= resx; x++)
            {
                ...
                for (GLuint l = 0; l <= 3; ++l)
                {
                    ...
                    bernstein_polynomial (l,3,t) * controlPoints[k*4 + l];
