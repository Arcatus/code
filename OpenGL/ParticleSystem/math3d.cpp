...
void Matrix3x3MakeRotation(Vector3* axis, float angle, Matrix3x3* result)
{
    ...
    result->m[1][0] = (1 - cosAngle) * axis->x * axis->y + sinAngle * axis->z;
...
void Matrix4x4MakeOrthographic(float left, float right, float bottom, float top,
    float near, float far, Matrix4x4* orthographic)
{
    Matrix4x4InitToZero(orthographic);

    orthographic->m[0][0] = 2 / (right - left);
    orthographic->m[0][3] = -(right + left) / (right - left);
    orthographic->m[1][1] = 2 / (top - bottom);
    orthographic->m[1][3] = -(top + bottom) / (top - bottom);
    orthographic->m[2][2] = -2 / (far - near);
    orthographic->m[2][3] = -(far + near) / (far - near);
...
{
    return binomial_coefficient(i, n) * powf(u, (float)i) * powf(1 - u, (float)(n - i));
}
