...
int main()
{
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        glUniform3f(glGetUniformLocation(shaderProgram, "lightPosition"),0.0f,5.0f,0.0f);
...
GLuint createBezierSurface(GLuint* vbo, GLuint* ebo, GLuint resx, GLuint resz, GLuint numcomp)
{
    ...
    for(GLfloat s = 0; s <= 1; s+=inc_s)
    {
        for(GLfloat t = 0; t <= 1; t+=inc_t)
        {
            ...
            vertices[index * numcomp + 3] =  0.0f;
            vertices[index * numcomp + 4] =  0.1f;
            vertices[index * numcomp + 5] =  0.3f;
    ...
    calcNormals(elements, resx * resz * 6, vertices, (resx + 1) * (resz + 1), numcomp);
    ...
    glBufferData(GL_ARRAY_BUFFER, 
        (resx + 1) * (resz + 1) * numcomp * sizeof(GLfloat), vertices, GL_STATIC_DRAW);
    ...
    glVertexAttribPointer(1, 4, GL_FLOAT, GL_FALSE, numcomp * sizeof(GLfloat), (GLvoid*)(3*sizeof(GLfloat)));
    glEnableVertexAttribArray(1);

    glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, numcomp * sizeof(GLfloat), (GLvoid*)(6 *sizeof(GLfloat)));
