...
GLuint createGrid(GLuint* vbo, GLuint* ebo, GLuint resx, GLuint resz);
...
int main()
{
    ...
    GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "OpenGLApp", glfwGetPrimaryMonitor(), NULL);
...
//  glfwSetInputMode(window,GLFW_CURSOR,GLFW_CURSOR_DISABLED);
    ...
    vao = createGrid(&vbo,&ebo,resx,resz);
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        glUniformMatrix4fv(glGetUniformLocation(shaderProgram, "mvp"),1, GL_TRUE, (GLfloat*)mvp.m);
 
        glBindVertexArray(vao);
        glDrawArrays(GL_POINTS,0,(resx + 1) * (resz + 1) * 3);
