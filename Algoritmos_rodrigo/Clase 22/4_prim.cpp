...
vector<int> prim(int i, vector<pair<int, int>> adyacencia[], int v)
{
   ...
   cola.push({ 0, i, i });                // de la cola irá saliendo la tripleta con la arista más barata; como caso especial, el vértice inicial puede incorporarse a costo 0 por sí mismo

   do {
      auto [costo, actual, previo] = cola.top( );    // ¡C++17!
      cola.pop( );
      if (predecesores[actual] != -1) {              // si el vértice ya estaba incorporado al árbol, lo ignoramos
         continue;
      }

      predecesores[actual] = previo;                     // si el vértice no estaba incorporado al árbol, registramos que lo incorporamos usando el predecesor sugerido
      for (auto [costo, vecino] : adyacencia[actual]) {  // ¡C++17! iterativamente inspeccionamos a sus vecinos
         cola.push({ costo, vecino, actual });           // nuestro vecino podría incorporarse con la arista que estamos viendo en este momento y el predecesor seríamos nosotros
      }
   } while (!cola.empty( ));
