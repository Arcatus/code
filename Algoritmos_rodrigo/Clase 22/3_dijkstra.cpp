...
vector<int> dijkstra(int i, vector<pair<int, int>> adyacencia[], int v)
{
   ...
   do {
      auto [distancia, actual] = cola.top( );   // ¡C++17!
      cola.pop( );
      if (distancias[actual] != INT_MAX) {      // si ya habíamos llegado a este vértice antes, lo ignoramos (la primera vez llegamos con menos distancia)
         continue;
      }

      distancias[actual] = distancia;           // si es la primera vez que llegamos a este vértice, entonces quitamos el INT_MAX y registramos su distancia real
      for (auto [costo, vecino] : adyacencia[actual]) {    // ¡C++17! iterativamente inspeccionamos a sus vecinos
         cola.push({ costo + distancia, vecino });         // si nosotros estamos a cierta distancia, nuestro vecino está a esa distancia + el costo de la arista
      }
   } while (!cola.empty( ));
