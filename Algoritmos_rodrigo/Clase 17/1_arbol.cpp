...
void inserta(nodo*& n, int v)       // tomamos el apuntador por referencia, si es modificado entonces el llamante podrá ver el cambio
{
   ...
   } else if (v < n->valor) {       // si n apunta a un nodo ya creado, entonces bajamos por el lado izquierdo si el valor que queremos insertar es menor al del nodo actual
      inserta(n->izq, v);
   } else {                         // si n apunta a un nodo ya creado, entonces bajamos por el lado derecho si el valor que queremos insertar no es menor al del nodo actual
      inserta(n->der, v);
   }
}
...
bool busca(const nodo* n, int v)    // tomamos el apuntador por valor, es barato de copiar y no necesitamos modificarlo
{
   if (n == nullptr) {              // nos piden buscar y llegamos a un apuntador nulo, entonces el valor no está
      ...
      return true;
   } else if (v < n->valor) {       // si n apunta a un nodo ya creado, entonces bajamos por el lado izquierdo si el valor que buscamos es menor al del nodo actual
      return busca(n->izq, v);
   }  else {                        // si n apunta a un nodo ya creado, entonces bajamos por el lado derecho si el valor que buscamos no es menor al del nodo actual
