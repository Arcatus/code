...
begin
     ...
     for i:=0 to MAXNIVELES do begin
         ...
         if (nodoFin = inicios[i]) or (siguientes[i] = finales[i]) then continue;
         if camino[i]^.hder = siguientes[i] then begin // avanza por la izquierda

            if nodoFin = siguientes[i] then begin // SOLO QUEDA UN NODO DESPUES, HAY QUE VOLTEARLO
               if siguientes[i]^.hizq = camino[i] then begin
                  siguientes[i]^.hizq:=finales[i];
                  siguientes[i]^.hder:=camino[i];
                  hijosNodoFin:=siguientes[i]^.cder;
               end
               else begin
                    siguientes[i]^.hder:=finales[i];
                    siguientes[i]^.hizq:=camino[i];
                    hijosNodoFin:=siguientes[i]^.cizq;
               end;
            end
            else begin
