...
int main() {
    ...
    int cnt = 0;
    bigint l, r, qq, pp, modd(1000000000000000009), x, cero(0);
    ll uu, oo;
    for(ll i=0; i<omega; ++i) { 
        //cnt++; if (cnt == 100) return 0;
        cin >> oo >> uu;
        qq = uu;
        pp = uu+1;
        bigint s = ((qq*(pp))/2)%modd;
        qq = oo;
        pp = oo-1;
        bigint z = (((pp)*qq)/2)%modd;
        //cout << s << " " << z << '\n';
        s = (s - z);
        //s /= 2;
        //if ( s < cero ) s+=modd;
