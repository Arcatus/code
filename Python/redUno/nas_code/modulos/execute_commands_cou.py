                ...
                for flujo in flujo_list:
                    ...
                    for each_command in command_list:
                        ...
                        if each_command.tipoComando.idCatalogoNivelUno == 4 \
                            ...
                            if each_command.idComando == ID_COMANDO_CONF_JUN:
                                resp = connection_dev.get_config("set")
                            else:
                        ...
                        #Comando con ID 214 es monitor traffic count l2
                        elif each_command.idComando == 214:
                            registro_db = \
                                 connection_dev.monitor_traffic_count_l2(
                                                        each_command.comando)

                            self.loggerExc.debug("salida %s en %s"
                                              % (registro_db, self.hostname))

                        #Comando con ID 206 es clear arp interface
                        elif each_command.idComando == 206:
                            self.loggerExc.debug("COMMAND: %s" % (
                                                        each_command.comando))
                            registro_db = \
                                 connection_dev.clear_arp_interface(
                                                        each_command.comando)
                            self.loggerExc.debug("RESPONSE: %s" % (registro_db))
                            self.loggerExc.debug("salida %s en %s"
                                              % (registro_db, self.hostname))

                        #Comando con ID 213 es clear interfaces statistics
                        elif each_command.idComando == 213:
                            registro_db = \
                                 connection_dev.clear_interfaces_statistics(
                                                          each_command.comando)
                            self.loggerExc.debug("salida %s en %s"
                                              % (registro_db, self.hostname))
