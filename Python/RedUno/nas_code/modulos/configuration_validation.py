# -*- coding: utf-8 -*-
__author__ = 'Jorge Campos, Arturo Herrera'
__copyright__ = 'Copyright 2019, UniNet'
__version__ = '2.0.0'
__email__ = 'arturohv@reduno.com.mx, jcpelcas@reduno.com.mx'
...
class ConfigurationValidation:
    ...
    """
    Clase que realiza la validacion de configuracion en el router.
    ...
    def validate_config(self, flujo_list, id_metodo):
        """
        Método utilizado para verificar la configuracion en los equipos.
        Aqui se van ejecutando los comandos derivados del flujo seleccionado
        por el usuario.

        Args:
            command_list_to_validate: Lista de comandos con su plantilla o 
            validacion para.
        ...
        try:
            #Banderas 
            #Error de Plantilla
            error_val_plant = False
            #Error en flujo
            flag_error_flow = False
            

            self.loggerVal.debug("INICIA::ValidationConfig::validator")
            ...
            self.loggerVal.debug("Conectividad via NETCONF %s en %s" 
                                        % (str(cxn_netconf), self.hostname))

            #cxn_netconf = False
            cxn_telnet = probe_connection(self.ip, 23)

            self.loggerVal.debug("Conectividad via Telnet %s en %s" 
            ...
            if cxn_telnet is True:
                ...
                for flujo in flujo_list:
                    ...
                    self.loggerVal.info("INICIO de validacion:: flujo \
                    ...
                    for each_command in command_list:
                        ...
                        if self.proveedor == "Juniper":
                            config = connection_dev.get_config("xml")
                        else: 
                            config = connection_dev.get_config("xml", 
                                   each_command.comando, each_command.segundos)

                        #Inicia la validacion del comando.
                        for validacion in each_command.validaciones:
                            if cxn_netconf is False:
                                self.loggerVal.debug("Validacion Telnet en %s"
                                                         % (self.hostname))

                                res_val, diff = comparador_telnet(config,
                                validacion.validacion,
                                validacion.tipoValidacion.idCatalogoNivelUno,
                                                                self.proveedor)


                                self.loggerVal.debug("Diff \n%s\n, en %s\n"
                                                     % (diff, self.hostname))

                                # id_Ejecucion 81 = Validacion de Configuracion 
                                # de Policy de IDE bajo demanda  
                                if self.id_ejecucion == 81 and type(res_val
                                                                    ) is int:
                                ...
                                self.loggerVal.debug("Validacion Netconf en %s"
                                                         % (self.hostname))

                                res_val = validation(config,
                                   validacion.validacion, 
                                   validacion.tipoValidacion.idCatalogoNivelUno,
                            ...
                            if res_val is True:
                                resultado.append(str(validacion.descripcion) +
                                                                    ' [OK]\r\n')
                            elif res_val is False:
                                error_val_plant = True
                                resultado.append(str(validacion.descripcion)
                                                             + ' [Error]\r\n')
                            else:
                                ...
                                #en el xml.
                                if res_val:
                                    if type(res_val) is float:
                                        resultado.append(str(
                                                        validacion.descripcion)
                                          + ': ' + str(int(res_val)) + '\n')
                                    elif type(res_val) is list:
                                        for val in res_val:
                                            resultado.append(
                                              str(validacion.descripcion) + 
                                                            val.text + "\n")
                                    else:
                                        resultado.append(str(
                                            validacion.descripcion) + res_val + "\n")
                                else:
                                    resultado.append(str(validacion.descripcion)
                    ...
                    self.loggerVal.info("Fin de validacion:: flujo \
                                        %s en equipo %s" % (flujo.nombreFlujo, 
                                                                self.hostname))
                                                                
                    #Si error_flag es verdadero coloca flag_error_flow en true
                    if error_val_plant is True:
                        flag_error_flow = True
                
            else:
                
                self.loggerVal.error("No se puede conectar a equipo,  %s" 
                                                            % (self.hostname))
            ...
            self.loggerVal.debug("FIN::ValidationConfig::validator")

            return json_response

        except Exception as error:
            self.loggerVal.error("Se ha generado una excepcion" +
                                            " en ValidationConfig %s" % (error))
